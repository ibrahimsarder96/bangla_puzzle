...
const totalTaka = document.querySelector('.totalTaka span');
...
const addCartToHTML = () =>{
  ...
  if(carts.length > 0){
    carts.forEach(cart => {
      ...
      taka = info.price * totalQuantity;
      newCart.innerHTML = `
      <div class=" relative mt-8 mx-5 flex items-center space-x-4 border-2 border-white rounded-md">
      <div>
      ...
      <div class="my-4">
        ...
        <div class="flex">
          <button>
            ...
            <span class="bg-white py-1 px-4">${cart.quantity}</span>
            <button>
      ...
      <i class="removeCart pointer fa fa-trash-o mr-8 p-2 rounded-md bg-white text-red-500 absolute -right-4 -top-4" style="font-size:26px"></i>
</div>
      `;
